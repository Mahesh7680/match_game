...
const randomChoice = imagesList[Math.floor(Math.random() * imagesList.length)]
...
class Fruits extends Component {
  state = {
    activeTabId: '',
    customAppsList: imagesList,
    score: 0,
    countDown: 60,
    timerRunning: false,
    triggeredImage: true,
    fullScreenImage: randomChoice,
  }
  ...
  onClickTabItem = tabId => {
    this.setState({
      activeTabId: tabId,
      customAppsList: imagesList.filter(
        each => each.category.includes(tabId) && each,
      ),
    })
  }
  ...
  onClickImageElement = id => {
    ...
    const randomChoiceUpdate =
      imagesList[Math.floor(Math.random() * imagesList.length)]

    const selectedSameImage = fullScreenImage.id === id
    if (selectedSameImage) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        fullScreenImage: randomChoiceUpdate,
      }))
    } else {
      this.setState(prevState => ({
  ...
  render() {
    ...
    console.log(triggeredImage)
    const fullScreenImageUrl = fullScreenImage.imageUrl
        ...
        <ul className="tab-items-container">
          {customAppsList.map(each => (
            <AppItem
              ...
              onClickImageElement={this.onClickImageElement}
            />
          ))}
        </ul>
      </div>
